...
void WeaponData_Init(void)
{
	...
	buffer = (char *)g_engfuncs.pfnLoadFileForMe("configs/weapon.vdf", &length);

	if (!buffer)
	{
		ALERT(at_error, "Couldn't to load weapon !\n");
		return;
	}
	...
	{
		ALERT(at_error, "Couldn't to parse weapon !\n");
	}
	...
	for (NewKeyValues *dat = pkvd; dat; dat = dat->GetNextKey())
	{
		...
		// ENGINE STRING
			if (info.szWeaponClass[0])
				info.iszWeaponClass = ALLOC_STRING(info.szWeaponClass);

			if (info.szModelFileName[0])
				info.iszModelFileName = ALLOC_STRING(info.szModelFileName);

			if (info.szPViewModelFileName[0])
				info.iszPViewModelFileName = ALLOC_STRING(info.szPViewModelFileName);

			if (info.szGViewModelFileName[0])
				info.iszGViewModelFileName = ALLOC_STRING(info.szGViewModelFileName);

		// PRECACHE
			if (info.iszModelFileName)
				PRECACHE_MODEL(STRING(info.iszModelFileName));

			if (info.iszPViewModelFileName)
				PRECACHE_MODEL(STRING(info.iszPViewModelFileName));

			if (info.iszGViewModelFileName)
				PRECACHE_MODEL(STRING(info.iszGViewModelFileName));

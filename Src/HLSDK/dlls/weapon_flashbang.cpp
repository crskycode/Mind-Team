...
void CFlashbang::Spawn(void)
{
	...
	m_iWeaponClass = WeaponClass_Flashbang;
...
void CFlashbang::WeaponIdle(void)
{
	...
	if (m_bStartThrow)
	{
		m_pPlayer->MessageAudio("Fireinthehole_Flashbang", FALSE);
		...
		CWorldFlashbang::Create(m_pPlayer->pev, m_Info.iszModelFileName, vecThrowStart, vecThrowVelocity, m_Info.m_GrenadeThrow.flGravity, m_Info.m_GrenadeThrow.flFriction, m_Info.m_GrenadeThrow.flAirTime, m_Info.iWeaponIndex);

...
static bool FindDescendingLadderApproachPoint( const CNavLadder *ladder, const CNavArea *area, Vector *pos )
{
	*pos = ladder->m_top - ladder->GetNormal() * 2.0f * HalfHumanWidth;
	...
	TraceResult result;
	UTIL_TraceLine( ladder->m_top, *pos, ignore_monsters, dont_ignore_glass, NULL, &result );
...
void CCSBot::ComputeLadderEndpoint( bool isAscending )
{
	...
	UTIL_TraceLine( from, m_pathLadder->m_bottom, ignore_monsters, dont_ignore_glass, NULL, &result );
...
bool CCSBot::UpdateLadderMovement( void )
{
	...
	if (gpGlobals->time - m_pathLadderTimestamp > ladderTimeoutDuration && !cv_bot_debug.GetBool())
	{

...
ConVar bot_show_occupy_time( "bot_show_occupy_time", "0", FCVAR_SERVER, "Show when each nav area can first be reached by each team." );

void DrawBattlefront( void );
ConVar bot_show_battlefront( "bot_show_battlefront", "0", FCVAR_SERVER, "Show areas where rushing players will initially meet." );

int UTIL_CSSBotsInGame( void );

ConVar bot_join_delay( "bot_join_delay", "0", FCVAR_SERVER, "Prevents bots from joining the server for this many seconds after a map change." );
...
void CCSBotManager::RestartRound( void )
{
	...
	SetLooseBomb( NULL );
	m_isBombPlanted = false;
	m_earliestBombPlantTimestamp = gpGlobals->time + RandomFloat( 10.0f, 30.0f ); // 60
	m_bombDefuser = NULL;
...
//!	ResetRadioMessageTimestamps();
	...
	m_roundStartTimestamp = gpGlobals->time ;//+ mp_freezetime.GetFloat();
 ...
 */
bool CCSBotManager::IsWeaponUseable( const CBasePlayerWeapon *weapon ) const
{
	if (weapon == NULL)
		return false;
 ...
 */
bool CCSBotManager::IsOnDefense( const CBasePlayer *player ) const
{
	switch (GetScenario())
	{
 ...
 */
bool CCSBotManager::IsOnOffense( const CBasePlayer *player ) const
{
...
//!	}
	...
	TheNavMesh->SetPlayerSpawnName( "info_player_start" );
...
//--------------------------------------------------------------------------------------------------------------
CON_COMMAND_F( bot_add, "bot_add <t|ct> <type> <difficulty> <name> - Adds a bot matching the given criteria." )
{
...
//!	BotArgumentsFromArgv( args, &name, &weaponType, &difficulty, &team );
	TheCSBots()->BotAddCommand( 0, FROM_CONSOLE, NULL, 0, BOT_EASY );
}


//--------------------------------------------------------------------------------------------------------------
CON_COMMAND_F( bot_add_t, "bot_add_t <type> <difficulty> <name> - Adds a terrorist bot matching the given criteria." )
{
	if ( !UTIL_IsCommandIssuedByServerAdmin() )
		return;
...
//!	BotArgumentsFromArgv( args, &name, &weaponType, &difficulty );
	TheCSBots()->BotAddCommand( TEAM_TERRORIST, FROM_CONSOLE, NULL, 0, BOT_EASY );
}


//--------------------------------------------------------------------------------------------------------------
CON_COMMAND_F( bot_add_ct, "bot_add_ct <type> <difficulty> <name> - Adds a Counter-Terrorist bot matching the given criteria." )
{
	if ( !UTIL_IsCommandIssuedByServerAdmin() )
		return;
...
//!	CSWeaponType weaponType;
//!	BotArgumentsFromArgv( args, &name, &weaponType, &difficulty );
	TheCSBots()->BotAddCommand( TEAM_CT, FROM_CONSOLE, NULL, 0, BOT_EASY );
}
...
//--------------------------------------------------------------------------------------------------------------
CON_COMMAND_F( bot_kill, "bot_kill <all> <t|ct> <type> <difficulty> <name> - Kills a specific bot, or all bots, matching the given criteria." )
{
	if ( !UTIL_IsCommandIssuedByServerAdmin() )
		return;
...
//--------------------------------------------------------------------------------------------------------------
CON_COMMAND_F( bot_kick, "bot_kick <all> <t|ct> <type> <difficulty> <name> - Kicks a specific bot, or all bots, matching the given criteria." )
{
//!	if ( !UTIL_IsCommandIssuedByServerAdmin() )
//!		return;
...
//--------------------------------------------------------------------------------------------------------------
CON_COMMAND_F( bot_knives_only, "Restricts the bots to only using knives" )
{
	if ( !UTIL_IsCommandIssuedByServerAdmin() )
		return;
...
//--------------------------------------------------------------------------------------------------------------
CON_COMMAND_F( bot_pistols_only, "Restricts the bots to only using pistols" )
{
	if ( !UTIL_IsCommandIssuedByServerAdmin() )
		return;
...
//--------------------------------------------------------------------------------------------------------------
CON_COMMAND_F( bot_snipers_only, "Restricts the bots to only using sniper rifles" )
{
	if ( !UTIL_IsCommandIssuedByServerAdmin() )
		return;
...
//--------------------------------------------------------------------------------------------------------------
CON_COMMAND_F( bot_all_weapons, "Allows the bots to use all weapons" )
{
	if ( !UTIL_IsCommandIssuedByServerAdmin() )
		return;
...
//--------------------------------------------------------------------------------------------------------------
CON_COMMAND_F( bot_goto_mark, "Sends a bot to the selected nav area (useful for testing navigation meshes)" )
{
	if ( !UTIL_IsCommandIssuedByServerAdmin() )
		return;
...
bool CCSBotManager::ClientCommand( CBasePlayer *player )
{
	return false;
}
 ...
 */
bool CCSBotManager::BotAddCommand( int team, bool isFromConsole, const char *profileName, int weaponType, BotDifficultyType difficulty )
{
...
//!	}
	...
	static BotProfile _staticProfile;
	// create the actual bot
	CCSBot *bot = CreateBot<CCSBot>( &_staticProfile, team );
	...
	bot->m_iJoinToTeam = 1;
	bot->m_iJoinToCharacter = 1;
	bot->m_iJoinToObserver = 0;
	bot->m_iJoiningState = GETINTOGAME;
...
void CCSBotManager::ExtractScenarioData( void )
{
	...
	for( i=1; i<gpGlobals->maxEntities; ++i )
	{
		...
		if (found)
		{
			if (m_zoneCount < MAX_ZONES)
			{
				...
				entity->WorldSpaceAABB( &absmin, &absmax );
	...
	{
		for( entity = UTIL_FindEntityByClassname( NULL, "info_player_start" );
			 entity && !FNullEnt( entity->edict() );
			 entity = UTIL_FindEntityByClassname( entity, "info_player_start" ) )
		{
			if (m_zoneCount < MAX_ZONES)
			{
				m_zone[ m_zoneCount ].m_isBlocked = false;
	...
	for( i=0; i<m_zoneCount; ++i )
	{
		...
		{
			Vector absmin, absmax;
			zone->m_entity->WorldSpaceAABB( &absmin, &absmax );
...
{
	CBaseEntity *pSpot = SelectSpawnSpot( "info_player_deathmatch" );
	if ( !pSpot )
		pSpot = SelectSpawnSpot( "info_player_start" );
		...
		if ( cv_bot_debug.GetInt() == 5 )
		{
			if ( m_zone[i].m_isBlocked )
				DevMsg( "%.1f: Zone %d, area %d (%.0f %.0f %.0f) is blocked from spawn area %d (%.0f %.0f %.0f)\n",
					gpGlobals->time, i, m_zone[i].m_area[0]->GetID(),
...
//!		collideable->WorldSpaceSurroundingBounds( &m_breakableExtent.lo, &m_breakableExtent.hi );
		m_breakableExtent.lo = m_breakable->v.absmin;
		m_breakableExtent.hi = m_breakable->v.absmax;
...
//		case SCENARIO_DEFUSE_BOMB:
//		{
//			if (player->m_iTeam == TEAM_TERRORIST && player->HasC4())
//				return true;
//
...
//		case SCENARIO_ESCORT_VIP:
//		{
//			if (player->m_iTeam == TEAM_CT && player->IsVIP())
//				return true;
//
//			return false;
//		}
...
//			if (bot->m_iTeam == TEAM_TERRORIST && bot->HasC4())
//				return 1;
//
//			break;
//		}
...
//			if (bot->m_iTeam == TEAM_CT && bot->m_bIsVIP)
//				return 1;
//
//			break;
//		}
...
//			if (bot->m_iTeam == TEAM_CT && bot->GetHostageEscortCount())
//				return 1;
//
//			break;
//		}
...
CBaseEntity *CCSBotManager::GetRandomSpawn( int team ) const
{
	...
	{
		// collect T spawns
		for( spot = UTIL_FindEntityByClassname( NULL, "info_player_deathmatch" );
			 spot;
			 spot = UTIL_FindEntityByClassname( spot, "info_player_deathmatch" ) )
		{
	...
	{
		// collect CT spawns
		for( spot = UTIL_FindEntityByClassname( NULL, "info_player_start" );
			 spot;
			 spot = UTIL_FindEntityByClassname( spot, "info_player_start" ) )
		{
			spawnSet.AddToTail( spot );			
		}
	}
...
CON_COMMAND_F( nav_check_connectivity, "Checks to be sure every (or just the marked) nav area can get to every goal area for the map (hostages or bomb site)." )
{

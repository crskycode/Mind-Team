...
ConVar nav_slope_limit( "nav_slope_limit", "0.7", FCVAR_SERVER, "The ground unit normal's Z component must be greater than this for nav areas to be generated." );
ConVar nav_restart_after_analysis( "nav_restart_after_analysis", "1", FCVAR_SERVER, "When nav nav_restart_after_analysis finishes, restart the server.  Turning this off can cause crashes, but is useful for incremental generation." );
 ...
 */
inline CNavArea *findFirstAreaInDirection( const Vector *start, NavDirType dir, float range, float beneathLimit, edict_t *traceIgnore = NULL, Vector *closePos = NULL )
{
	CNavArea *area = NULL;
...
//!		UTIL_TraceLine( *start, pos, MASK_PLAYERSOLID_BRUSHONLY, traceIgnore, COLLISION_GROUP_NONE, &result );
		UTIL_TraceLine( *start, pos, ignore_monsters, dont_ignore_glass, traceIgnore, &result );
...
//!	trace_t result;
	...
	while( (entity = UTIL_FindEntityByClassname( entity, "func_ladder" )) != NULL )
	{
...
void CNavMesh::BeginGeneration( bool incremental )
{
	...
	CBaseEntity *spawn = UTIL_FindEntityByClassname( NULL, GetPlayerSpawnName() );
...
//!		UTIL_TraceLine( center + Vector( 0, 0, fudge ), tryPos + Vector( 0, 0, fudge ), MASK_PLAYERSOLID_BRUSHONLY, NULL, COLLISION_GROUP_NONE, &result );
		UTIL_TraceLine( center + Vector( 0, 0, fudge ), tryPos + Vector( 0, 0, fudge ), ignore_monsters, dont_ignore_glass, NULL, &result );
		if (result.flFraction != 1.0f || result.fStartSolid)
			continue;
...
//!				UTIL_TraceLine( fromOrigin, toOrigin, MASK_PLAYERSOLID_BRUSHONLY, &filter, &result );
				UTIL_TraceLine( fromOrigin, toOrigin, ignore_monsters, dont_ignore_glass, NULL, &result );
				...
				{
					if (IsEntityWalkable( result.pHit, WALK_THRU_EVERYTHING ))
					{
						walkable = true;
					}
					else
...
//!	CTraceFilterWalkableEntities traceFilter( NULL, COLLISION_GROUP_NONE, flags );
	...
	result.flFraction = 0.0f;

	const int maxTries = 50;
	for( int t=0; t<maxTries; ++t )
	{
//!		UTIL_TraceLine( useFrom, to, MASK_PLAYERSOLID, &traceFilter, &result );
		UTIL_TraceLine( useFrom, to, ignore_monsters, dont_ignore_glass, ignore, &result );

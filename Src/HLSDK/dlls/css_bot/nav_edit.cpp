...
ConVar nav_show_area_info( "nav_show_area_info", "0.5", FCVAR_SERVER, "Duration in seconds to show nav area ID and attributes while editing" );
ConVar nav_snap_to_grid( "nav_snap_to_grid", "0", FCVAR_SERVER, "Snap to the nav generation grid when creating new nav areas" );
ConVar nav_create_place_on_ground( "nav_create_place_on_ground", "0", FCVAR_SERVER, "If true, nav areas will be placed flush with the ground when created by hand." );
...
//!	UTIL_TraceLine( from, to, MASK_PLAYERSOLID_BRUSHONLY, player, COLLISION_GROUP_NONE, &result );
	UTIL_TraceLine( from, to, ignore_monsters, dont_ignore_glass, player->edict(), &result );
...
//!			m_climbableSurface = physprops->GetSurfaceData( result.surface.surfaceProps )->game.climbable != 0;
			m_climbableSurface = POINT_CONTENTS( result.vecEndPos ) == CONTENTS_LADDER;
//!			if ( !m_climbableSurface )
//!			{
//!				m_climbableSurface = (result.contents & CONTENTS_LADDER) != 0;
//!			}
			m_surfaceNormal = result.vecPlaneNormal;
		...
		m_editCursorPos = result.vecEndPos;
...
//!				Vector start = result.startpos;
				Vector start = from;
				Vector end = result.vecEndPos + 10.0f * dir; // extend a few units into the ground
				Ray_t ray;
			...
			if ( !m_selectedArea )
			{
				m_selectedArea = TheNavMesh->GetNearestNavArea( result.vecEndPos, false, 500.0f );
			}
		}
		...
		if ( m_selectedArea )
		{
			...
			{
				m_splitEdge = SnapToGrid( result.vecEndPos.y, true );
				m_splitAlongX = true;
			}
			else
			{
				m_splitEdge = SnapToGrid( result.vecEndPos.x, true );
				m_splitAlongX = false;
			}
		}
...
//!	UTIL_TraceLine( start, end, MASK_PLAYERSOLID_BRUSHONLY, NULL, COLLISION_GROUP_NONE, &result );
	UTIL_TraceLine( start, end, ignore_monsters, dont_ignore_glass, NULL, &result );

	bool climbableSurface = false;
	if (result.flFraction != 1.0f)
	{
//!		climbableSurface = physprops->GetSurfaceData( result.surface.surfaceProps )->game.climbable != 0;
		climbableSurface = POINT_CONTENTS( result.vecEndPos ) == CONTENTS_LADDER;
//!		if ( !climbableSurface )
//!		{
...
//!		UTIL_TraceLine( on, out, MASK_PLAYERSOLID_BRUSHONLY, NULL, COLLISION_GROUP_NONE, &result );
		UTIL_TraceLine( on, out, ignore_monsters, dont_ignore_glass, NULL, &result );
	...
	if ( GetActiveNavArea() )
	{
		...
		if ( m_selectedLadder )
		{
			...
			if (m_selectedLadder != m_lastSelectedLadder || nav_show_area_info.GetBool())
			{
				...
				_snprintf( buffer, sizeof( buffer ), "Ladder #%d\n", m_selectedLadder->GetID() );
		...
		if (m_selectedArea)
		{
			...
			if (m_showAreaInfoTimer.HasStarted() && !m_showAreaInfoTimer.IsElapsed() )
			{
				...
				_snprintf( buffer, sizeof( buffer ), "Area #%d %s %s\n", m_selectedArea->GetID(), locName, attrib );
...
void CNavMesh::CommandNavSplit( void )
{
	...
	if ( m_selectedArea )
	{
		m_selectedArea->SplitEdit( m_splitAlongX, m_splitEdge );
	}
...
void CNavMesh::CommandNavMakeSniperSpots( void )
{
	...
	if ( m_selectedArea )
	{
		...
		MakeSniperSpots( m_selectedArea );
	}
...
void CNavMesh::CommandNavMerge( void )
{
	...
	if ( m_selectedArea )
	{
		...
		{
			m_selectedArea->MergeEdit( GetMarkedArea() );
		}
		else
		{
...
void CNavMesh::CommandNavMark( void )
{
	...
	{
		const char *areaIDNameToMark = CMD_ARGV(1);
...
void CNavMesh::CommandNavSplice( void )
{
	...
	if ( m_selectedArea )
	{
		if (GetMarkedArea())
		{
			m_selectedArea->SpliceEdit( GetMarkedArea() );
		}
		else
		{
...
void CNavMesh::CommandNavWarpToMark( void )
{
	...
	if (GetMarkedArea())
	{
		if ( ( !player->IsAlive() || player->pev->iuser1 ) && player->pev->iuser1 == OBS_ROAMING )
		{

...
void AttackState::OnEnter( CCSBot *me )
{
	...
	m_pinnedDownTimestamp = gpGlobals->time + RandomFloat( 7.0f, 10.0f );

	m_shieldToggleTimestamp = gpGlobals->time + RandomFloat( 2.0f, 10.0f );
		...
		if (!m_crouchAndHold)
		{
			if (enemy)
			{
				...
				if (RandomFloat( 0.0f, 100.0f ) < crouchChance)
				{
					...
					UTIL_TraceLine( origin, enemy->EyePosition(), ignore_monsters, dont_ignore_glass, me->edict(), &result );
...
void AttackState::Dodge( CCSBot *me )
{
	...
	if (m_shouldDodge && !me->IsUsingSniperRifle() && !m_crouchAndHold)
	{
		...
		else if (gpGlobals->time >= m_nextDodgeStateTimestamp)
		{
			...
			m_nextDodgeStateTimestamp = gpGlobals->time + RandomFloat( 0.3f, 1.0f );
			m_firstDodge = false;
		}
...
/*	CWeaponCSBase *weapon = me->GetActiveCSWeapon();
	...
	/*if (me->HasShield())
	{
		...
		{
			m_shieldToggleTimestamp = gpGlobals->time + RandomFloat( 0.5, 2.0f );
...
//			me->GetChatter()->KilledMyEnemy( enemy->entindex() );
	...
	if (true || gpGlobals->time > m_reacquireTimestamp)
		me->FireWeaponAtEnemy();

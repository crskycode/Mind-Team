...
public:
	...
	void SetLeader( CBasePlayer *player )			{ m_leader = player; }
...
private:
	EHANDLE m_funcDoor;									///< the func_door we are opening
	EHANDLE m_propDoor;									///< the prop_door we are opening
	bool m_isDone;
...
public:
	...
	CBasePlayer *GetFollowLeader( void ) const;					///< return the leader we are following
...
//!	bool RespondToHelpRequest( CBasePlayer *player, Place place, float maxRange = -1.0f );	///< decide if we should move to help the player, return true if we will
//!	bool IsUsingVoice() const;									///< new-style "voice" chatter gets voice feedback
	...
	void SetBotEnemy( CBasePlayer *enemy );						///< set given player as our current enemy
	CBasePlayer *GetBotEnemy( void ) const;
	...
	float GetTravelDistanceToPlayer( CBasePlayer *player ) const;	///< return shortest path travel distance to this player	
	bool DidPlayerJustFireWeapon( const CBasePlayer *player ) const;	///< return true if the given player just fired their weapon
	...
	bool IsVisible( CBasePlayer *player, bool testFOV = false, unsigned char *visParts = NULL ) const;	///< return true if we can see any part of the player
	...
	const Vector &GetPartPosition( CBasePlayer *player, VisiblePartType part ) const;	///< return world space position of given part on player
	...
	bool IsUsing( int weapon ) const;								///< returns true if using the specific weapon
	...
	CBasePlayerWeapon *GetActiveCSWeapon( void ) const;				///< get our current Counter-Strike weapon
...
//!	virtual void Event_Killed( const CTakeDamageInfo &info );
	...
	virtual bool BumpWeapon( CBaseEntity *pWeapon );		///< invoked when in contact with a CWeaponBox
...
//!	float m_hostageEscortCountTimestamp;
	...
	void ComputePartPositions( CBasePlayer *player );				///< compute part positions from bone location
...
//!	if (!m_enemy->IsAlive() && gpGlobals->time - GetEnemyDeathTimestamp() > (1.0f - 0.8f * GetProfile()->GetSkill()))
//!		return true;
...
inline CBasePlayerWeapon *CCSBot::GetActiveCSWeapon( void ) const
{
	return reinterpret_cast<CBasePlayerWeapon *>( GetActiveWeapon() );
}
...
inline CBasePlayer *CCSBot::GetFollowLeader( void ) const
{ 
	return *(EHANDLE *)( &m_leader );
}
...
inline CBasePlayer *CCSBot::GetBotEnemy( void ) const
{
	return m_enemy;
}
...
inline float CCSBot::GetTravelDistanceToPlayer( CBasePlayer *player ) const
{
	if (player == NULL)
		return -1.0f;
 ...
 */
inline bool IsSniperRifle( CBasePlayerWeapon *weapon )
{
//!	if (weapon == NULL)
//!		return false;
...
// Prototypes
//
extern int GetBotFollowCount( CBasePlayer *leader );
extern const Vector *FindNearbyRetreatSpot( CCSBot *me, float maxRange = 250.0f );
extern const HidingSpot *FindInitialEncounterSpot( CBasePlayer *me, const Vector &searchOrigin, float enemyArriveTime, float maxRange, bool isSniper );

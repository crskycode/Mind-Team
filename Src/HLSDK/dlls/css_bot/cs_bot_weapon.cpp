...
void CCSBot::FireWeaponAtEnemy( void )
{
	...
	if (gpGlobals->time > m_fireWeaponTimestamp &&
	...
	//		//gpGlobals->time > m_reacquireTimestamp &&
	//		IsEnemyVisible())
	//	{
	...
	//						m_fireWeaponTimestamp = RANDOM_FLOAT( 0.15f, 0.25f );		// 0.15, 0.5
	//					}
	//				}
	//			}
...
void CCSBot::SetAimOffset( float accuracy )
{
	...
	m_aimOffsetTimestamp = gpGlobals->time + RandomFloat( 0.25f, 1.0f ); // 0.25, 1.5f
}
 ...
 */
bool CCSBot::IsUsing( int weaponID ) const
{
...
{
	//CBasePlayerWeapon *weapon = GetActiveCSWeapon();
...
{
	//CBasePlayerWeapon *weapon = static_cast<CBasePlayerWeapon *>( Weapon_GetSlot( WEAPON_SLOT_RIFLE ) );
...
{
	//CBasePlayerWeapon *weapon = static_cast<CBasePlayerWeapon *>( Weapon_GetSlot( WEAPON_SLOT_PISTOL ) );
 ...
 */
bool CCSBot::DoEquip( CBasePlayerWeapon *weapon )
{
	if (weapon == NULL)
		return false;
...
void CCSBot::EquipBestWeapon( bool mustEquip )
{
	...
	//CBasePlayerWeapon *primary = static_cast<CBasePlayerWeapon *>( Weapon_GetSlot( WEAPON_SLOT_RIFLE ) );
	//if (primary)
	//{
	...
	//{
	//	if (DoEquip( static_cast<CBasePlayerWeapon *>( Weapon_GetSlot( WEAPON_SLOT_PISTOL ) ) ))
	//		return;
	//}
...
void CCSBot::EquipPistol( void )
{
	...
	//{
	//	CBasePlayerWeapon *pistol = static_cast<CBasePlayerWeapon *>( Weapon_GetSlot( WEAPON_SLOT_PISTOL ) );
	//	DoEquip( pistol );
	//}
}
...
{
	//CBasePlayerWeapon *grenade = static_cast<CBasePlayerWeapon *>( Weapon_GetSlot( WEAPON_SLOT_GRENADES ) );
...
{
	return ( GetActiveWeapon() && GetActiveWeapon()->m_flNextPrimaryAttack <= gpGlobals->time );
}
...
bool CCSBot::FindGrenadeTossPathTarget( Vector *pos )
{
	...
	UTIL_TraceLine( visibleSpot, check, ignore_monsters, dont_ignore_glass, edict(), &result );
...
void CCSBot::LookForGrenadeTargets( void )
{
	...
	if (tossArea->GetEarliestOccupyTime( enemyTeam ) > gpGlobals->time)
	{
...
public:
	...
	bool operator() ( CBasePlayer *player )
	{
		...
		if (m_me->InSameTeam( player ))
		{
			...
			{
				if (m_me->IsVisible( (CBasePlayer *)player ))
				{
...
void CCSBot::SilencerCheck( void )
{
	...
	//	if ( weapon->m_flNextSecondaryAttack >= gpGlobals->time )
	//		return;
 ...
 */
bool CCSBot::BumpWeapon( CBaseEntity *pWeapon )
{
	//CBasePlayerWeapon *droppedGun = dynamic_cast< CBasePlayerWeapon* >( pWeapon );

	//// right now we only care about primary weapons on the ground
	//if ( droppedGun && droppedGun->GetSlot() == WEAPON_SLOT_RIFLE )
	//{
	//	CBasePlayerWeapon *myGun = dynamic_cast< CBasePlayerWeapon* >( Weapon_GetSlot( WEAPON_SLOT_RIFLE ) );
...
bool CCSBot::IsFriendInLineOfFire( void )
{
	...
	TraceResult result;
	UTIL_TraceLine( EyePosition(), EyePosition() + 10000.0f * aimDir, ignore_monsters, dont_ignore_glass, edict(), &result );
...
float CCSBot::ComputeWeaponSightRange( void )
{
	...
	return (EyePosition() - result.vecEndPos).Length();
}
 ...
 */
bool CCSBot::DidPlayerJustFireWeapon( const CBasePlayer *player ) const
{
	// if this player has just fired his weapon, we notice him
	//CBasePlayerWeapon *weapon = player->GetActiveCSWeapon();
	//return (weapon && !weapon->IsSilenced() && weapon->m_flNextPrimaryAttack > gpGlobals->time);
	return false;
}

...
CBaseEntity * CheckForEntitiesAlongSegment( const Vector &start, const Vector &end, const Vector &mins, const Vector &maxs, CPushAwayEnumerator *enumerator )
{
	...
	ISpatialPartition::EnumerateElementsAlongRay( PARTITION_ENGINE_SOLID_EDICTS, ray, false, enumerator );
...
void CCSBot::BreakablesCheck( void )
{
	...
	*/
	...
	if ( IsLookingAtSpot( PRIORITY_HIGH ) && m_lookAtSpotAttack )
	{
		...
		else if ( GetActiveWeapon() && GetActiveWeapon()->m_flNextPrimaryAttack <= gpGlobals->time )
		{
			...
			if ( !shouldShoot )
			{
				...
				TraceResult result;
				UTIL_TraceLine( EyePosition(), EyePosition() + FarBreakableCheckDist * aimDir, ignore_monsters, edict(), &result );
				if ( !FNullEnt(result.pHit) )
				{
					LOSbreakable.EnumElement( CBaseEntity::Instance(result.pHit) );
...
{
	CBaseEntity *spot;
	const char *spawnName = (GetTeamNumber() == TEAM_TERRORIST) ? "info_player_start" : "info_player_deathmatch";

	// check if we are at any of the enemy's spawn points
	for( spot = UTIL_FindEntityByClassname( NULL, spawnName ); spot; spot = UTIL_FindEntityByClassname( spot, spawnName ) )
	{

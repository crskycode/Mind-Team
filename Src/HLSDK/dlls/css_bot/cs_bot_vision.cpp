...
ConVar bot_mimic( "bot_mimic", "0", FCVAR_SERVER );
//--------------------------------------------------------------------------------------------------------------
/**
...
//!	CTraceFilterNoNPCsOrPlayer traceFilter( ignore, COLLISION_GROUP_NONE );
	UTIL_TraceLine( EyePositionConst(), pos, ignore_monsters, dont_ignore_glass, ignore ? ignore->edict() : NULL, &result );
 ...
 */
void CCSBot::ComputePartPositions( CBasePlayer *player )
{
 ...
 */
const Vector &CCSBot::GetPartPosition( CBasePlayer *player, VisiblePartType part ) const
{
...
void CCSBot::UpdateLookAround( bool updateNow )
{
	...
	if (IsAtHidingSpot() || IsNotMoving( minStillTime ))
	{
		...
		if (IsUsingSniperRifle())
			m_lookAroundStateTimestamp = gpGlobals->time + RandomFloat( 5.0f, 10.0f );
		else
			m_lookAroundStateTimestamp = gpGlobals->time + RandomFloat( 1.0f, 2.0f );	// 0.5, 1.0
	...
	if (m_spotEncounter)
	{
		...
		if (!IsSafe() && !IsLookingAtSpot( PRIORITY_LOW ))
		{
			...
			m_spotCheckTimestamp = gpGlobals->time + asleep * RandomFloat( 10.0f, 30.0f );
...
//!	CTraceFilterNoNPCsOrPlayer traceFilter( this, COLLISION_GROUP_NONE );
	UTIL_TraceLine( eye, target, ignore_monsters, dont_ignore_glass, edict(), &result );
	...
	for( float angle = angleInc; angle <= angleLimit; angle += angleInc )
	{
		...
		for( int side=0; side<2; ++side )
		{
			...
			UTIL_TraceLine( eye, rotPoint, ignore_monsters, dont_ignore_glass, edict(), &result );
			...
			for( float bendLength=startLength; bendLength <= visibleLength; bendLength += bendStepSize )
			{
				...
				UTIL_TraceLine( bendPoint, target, ignore_monsters, dont_ignore_glass, edict(), &result );

				if (doDebug && cv_bot_debug.GetBool() && IsLocalPlayerWatchingMe())
				{
					NDebugOverlay::Line( bendPoint, result.vecEndPos, colorSet[color].r/2, colorSet[color].g/2, colorSet[color].b/2, true, debugDuration );
				}
 ...
 */
bool CCSBot::IsNoticable( const CBasePlayer *player, unsigned char visParts ) const
{
...
//			}
			...
			if (IsSniperRifle( player->GetActiveWeapon() ))
			{
...
void CCSBot::UpdateReactionQueue( void )
{
	...
	if (threat)
	{
		...
		m_enemyQueue[ now ].isReloading = false;//threat->IsReloading();
		m_enemyQueue[ now ].isProtectedByShield = false;//threat->IsProtectedByShield();
	}
	else
	{
...
void CCSBot::Blind( float holdTime, float fadeTime, float startingAlpha )
{
	...
	if (holdTime < mildBlindTime)
	{
		Wait( 0.75f * holdTime );
		BecomeAlert();
		CBot::Blind( holdTime, fadeTime, startingAlpha );
		return;
	}
...
void CCSBot::UpdatePanicLookAround( void )
{
	...
	const Vector &eyeAngles = EyeAngles();

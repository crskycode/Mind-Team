...
{
	m_clearedTimestamp[ teamID % MAX_NAV_TEAMS ] = gpGlobals->time;
}
...
//!	UTIL_TraceLine( eye, GetCenter() + Vector( 0, 0, offset ), MASK_BLOCKLOS_AND_NPCS, &traceFilter, &result );
	UTIL_TraceLine( eye, GetCenter() + Vector( 0, 0, offset ), ignore_monsters, ignore_glass, NULL, &result );
...
//!		UTIL_TraceLine( eye, corner + Vector( 0, 0, offset ), MASK_BLOCKLOS_AND_NPCS, &traceFilter, &result );
		UTIL_TraceLine( eye, corner + Vector( 0, 0, offset ), ignore_monsters, ignore_glass, NULL, &result );
		if (result.flFraction == 1.0f)
		{
			// we can see this area
			if (visSpot)

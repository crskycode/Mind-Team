...
ConVar nav_coplanar_slope_limit( "nav_coplanar_slope_limit", "0.99", FCVAR_SERVER );
ConVar nav_split_place_on_ground( "nav_split_place_on_ground", "0", FCVAR_SERVER, "If true, nav areas will be placed flush with the ground when split." );
ConVar nav_area_bgcolor( "nav_area_bgcolor", "0 0 0 30", FCVAR_SERVER, "RGBA color to draw as the background color for nav areas while editing." );
ConVar nav_corner_adjust_adjacent( "nav_corner_adjust_adjacent", "18", FCVAR_SERVER, "radius used to raise/lower corners in nearby areas when raising/lowering corners." );
...
//!	UTIL_TraceLine( from, to, MASK_PLAYERSOLID_BRUSHONLY, NULL, COLLISION_GROUP_NONE, &result );
	UTIL_TraceLine( from, to, ignore_monsters, dont_ignore_glass, NULL, &result );
...
//!				UTIL_TraceLine( eye, walkable, CONTENTS_SOLID|CONTENTS_MOVEABLE|CONTENTS_PLAYERCLIP, NULL, COLLISION_GROUP_NONE, &result );
				UTIL_TraceLine( eye, walkable, ignore_monsters, dont_ignore_glass, NULL, &result );
...
//!			UTIL_TraceLine( eye, Vector( spotPos.x, spotPos.y, spotPos.z + HalfHumanHeight ), MASK_PLAYERSOLID_BRUSHONLY, NULL, COLLISION_GROUP_NONE, &result );
			UTIL_TraceLine( eye, Vector( spotPos.x, spotPos.y, spotPos.z + HalfHumanHeight ), ignore_monsters, dont_ignore_glass, NULL, &result );
			if (result.flFraction != 1.0f)
				continue;
...
void CNavArea::IncreaseDanger( int teamID, float amount )
{
	...
	m_dangerTimestamp[ teamIdx ] = gpGlobals->time;
}
...
static ConCommand nav_update_blocked( "nav_update_blocked", CommandNavUpdateBlocked, "Updates the blocked/unblocked status for every nav area." );
...
void CNavArea::CheckWaterLevel( void )
{
	...
	pos.z += 1;
	m_isUnderwater = POINT_CONTENTS( pos ) == CONTENTS_WATER;
}
...
static ConCommand nav_check_floor( "nav_check_floor", CommandNavCheckFloor, "Updates the blocked/unblocked status for every nav area." );

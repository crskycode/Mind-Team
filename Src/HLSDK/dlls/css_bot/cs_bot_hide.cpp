...
void HideState::OnUpdate( CCSBot *me )
{
	...
	if (!me->IsReloading())
	{
		...
		if (me->IsFollowing())
		{
			CBasePlayer *leader = static_cast<CBasePlayer *>( static_cast<CBaseEntity *>( me->GetFollowLeader() ) );
...
*/
	...
	else
	{
		...
		float range;
		CBasePlayer *camper = static_cast<CBasePlayer *>( UTIL_GetClosestPlayer( m_hidingSpot, &range ) );
		...
		if (range < atDist)
		{
			...
			{
				UTIL_TraceLine( me->EyePosition(), me->EyePosition() + 1000.0f * Vector( BotCOS(angle), BotSIN(angle), 0.0f ), ignore_monsters, dont_ignore_glass, me->edict(), &result );

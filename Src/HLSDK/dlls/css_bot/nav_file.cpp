...
char *GetBspFilename( const char *navFilename )
{
	...
	_snprintf( bspFilename, sizeof( bspFilename ), "maps\\%s.bsp", STRING( gpGlobals->mapname ) );
...
void CNavArea::ComputeEarliestOccupyTimes( void )
{
	...
	for( spot = UTIL_FindEntityByClassname( NULL, "info_player_terrorist" );
		 spot;
		 spot = UTIL_FindEntityByClassname( spot, "info_player_terrorist" ) )
	{
	...
	for( spot = UTIL_FindEntityByClassname( NULL, "info_player_counterterrorist" );
		 spot;
		 spot = UTIL_FindEntityByClassname( spot, "info_player_counterterrorist" ) )
	{
		float travelDistance = NavAreaTravelDistance( spot->GetAbsOrigin(), m_center, cost );
		if (travelDistance < 0.0f)
			continue;
...
const char *CNavMesh::GetFilename( void ) const
{
	...
	_snprintf( filename, sizeof( filename ), "maps/%s.nav", STRING( gpGlobals->mapname ) );

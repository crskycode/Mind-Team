...
void CNavNode::Draw( void )
{
	...
	{
		char text[16];
		_snprintf( text, sizeof( text ), "%d", m_id );
...
//!	CTraceFilterWalkableEntities filter( NULL, COLLISION_GROUP_PLAYER_MOVEMENT, WALK_THRU_EVERYTHING );
	...
	for ( int i=0; i<NUM_CORNERS; ++i )
	{
		...
		UTIL_TraceHull(
			...
			actualGroundPos + Vector( 0, 0, VEC_IDLE_HULL_MAX[2] - 0.2f ),
			mins,
			maxs,
			ignore_monsters,
			NULL,
			&tr );
		actualGroundPos.z += /*tr.fractionleftsolid **/ VEC_IDLE_HULL_MAX[2];
		float maxHeight = actualGroundPos.z + VEC_DUCK_HULL_MAX.z;
		for ( ; tr.fStartSolid && actualGroundPos.z <= maxHeight; actualGroundPos.z += 1.0f )
		{
		...
		if ( nav_show_nodes.GetBool() )
		{
			if ( nav_test_node_crouch_dir.GetInt() == i || nav_test_node_crouch_dir.GetInt() == NUM_CORNERS  )
			{
				if ( tr.fStartSolid )
				{
					NDebugOverlay::Box( actualGroundPos, mins, maxs+Vector( 0, 0, VEC_IDLE_HULL_MAX[2]), 255, 0, 0, 10, 20.0f );
				}
				else if ( m_crouch[corner] )
				{
					NDebugOverlay::Box( actualGroundPos, mins, maxs+Vector( 0, 0, VEC_IDLE_HULL_MAX[2]), 0, 0, 255, 10, 20.0f );
				}
				else
				{
					NDebugOverlay::Box( actualGroundPos, mins, maxs+Vector( 0, 0, VEC_IDLE_HULL_MAX[2]), 0, 255, 0, 10, 10.0f );
				}
			}
		}
#endif // DEBUG_NAV_NODES

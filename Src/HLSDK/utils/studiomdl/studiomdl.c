...
#include "lbmlib.h"
#include "scriplib.h"
#include "./HLSDK/utils/common/mathlib.h"
#define EXTERN
...
// dword alignment for each scan
	...
	printf ("BMP %s [%d %d] (%.0f%%)  %6d bytes\n", ptexture->name,  ptexture->skinwidth, ptexture->skinheight, 
		...
		ptexture->size );
#define TEXTURE_SIZE	( 1024 * 1024 + ( 3 * 256 ) )
	if (ptexture->size > TEXTURE_SIZE)
	{
		printf("%.0f %.0f %.0f %.0f\n", ptexture->min_s, ptexture->max_s, ptexture->min_t, ptexture->max_t );
	...
	{
		memcpy( pdest, ptexture->ppicture, ptexture->skinwidth*ptexture->skinheight );
		pdest += ( ptexture->skinwidth * ptexture->skinheight );
	}
		...
		if (sscanf( line, "%d %f %f %f %f %f %f", &index, &pos[0], &pos[1], &pos[2], &rot[0], &rot[1], &rot[2] ) == 7)
		{
			if (t >= panim->startframe && t <= panim->endframe)
			{
				if (panim->node[index].parent == -1) {
					...
					if ( rotate[0] ) {
						rot[0] += rotate[0] * ( Q_PI / 180.0 );
					}
					if ( rotate[1] ) {
						rot[1] += rotate[1] * ( Q_PI / 180.0 );
					}
					if ( rotate[2] ) {
						rot[2] += rotate[2] * ( Q_PI / 180.0 );
					}
					// rotate model
					rot[2]			+= zrotation;
				}
...
void Cmd_SetTextureRendermode( void )
{
	...
	else if (!stricmp(token, "additive"))
	{
	...
	else if (!stricmp(token, "minimize"))
	{
		texture[iTextureIndex].flags |= STUDIO_NF_RESIZED;
		return;
	}
	else
	{

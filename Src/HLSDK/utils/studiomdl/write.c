...
#define RESIZED_TEXTURE	( 8 * 8 + ( 3 * 256 ) )
		static byte uTemp[ RESIZED_TEXTURE ] = { 0 };
		if ( !( texture[i].flags & STUDIO_NF_RESIZED ) ) {
			...
			ptexture[i].height		= 8;
			ptexture[i].index		= (pData - pStart);
			memcpy( pData, uTemp, RESIZED_TEXTURE );
			pData += RESIZED_TEXTURE;
		}
	}
	ALIGN( pData );
}

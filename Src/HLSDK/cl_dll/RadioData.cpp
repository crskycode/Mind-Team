...
void RadioData_Init(void)
{
	...
	buffer = (char *)gEngfuncs.COM_LoadFile("Configs/Radio.vdf", 5, &length);

	if (!buffer)
	{
		gEngfuncs.Con_Printf("Error: Couldn't to load radio !\n");
		return;
	}
	...
	{
		gEngfuncs.Con_Printf("Error: Couldn't to parse radio !\n");
	}

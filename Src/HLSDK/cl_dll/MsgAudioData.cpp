...
void MsgAudio_Init(void)
{
	...
	bufer = (char *)gEngfuncs.COM_LoadFile("Configs/MsgAudio.vdf", 5, &length);

	if (!bufer)
	{
		gEngfuncs.Con_Printf("Error: Couldn't to load message !\n");
		return;
	}
	...
	{
		gEngfuncs.Con_Printf("Error: Couldn't to parse message !\n");
	}

	gEngfuncs.COM_FreeFile(bufer);
...
void MsgAudio_GetFile(int iTeam, bool bIsWoman, bool bIsChinese, const char *pSentence, const char *&pSoundFile, int iRand)
{
	...
	if (!pSound->iNumFile || iRand < 0 || iRand > pSound->iNumFile - 1)
		return;

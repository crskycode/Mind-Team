...
#ifdef ENCODE_FILE
  DecodeData(m_pData,m_pData,m_nSize,0);
#endif
...
#ifdef ENCODE_FILE
  DecodeData(pBytes,pBytes,length,startOffset);
#endif
...
#ifdef ENCODE_FILE
    DecodeData(pBytes,pBytes,length,m_nCurPos);
#endif
    m_nCurPos += length;
    return length;
  }
...
BOOL CRezItm::Save() {
  ...
  BYTE *pBuff = new BYTE[m_nSize];
  ASSERT(pBuff != NULL);
  if (pBuff == NULL) return FALSE;
#ifdef ENCODE_FILE
  EncodeData(m_pData,pBuff,m_nSize,0);
#endif
  ...
  if (m_nFilePos == 0) {
    ...
    // write out the data
    if (m_pRezFile->Write(m_pParentDir->m_pRezMgr->m_nNextWritePos,0,m_nSize,pBuff) != m_nSize) {
      delete [] pBuff;
  ...
  // if this resource has been saved before (just write it where it belongs)
    ...
    // write out the data
    if (m_pRezFile->Write(m_nFilePos,0,m_nSize,pBuff) != m_nSize) {
      delete [] pBuff;
      ASSERT(FALSE); // failed to write out data
      return FALSE;
    }
...
CRezDir::CRezDir(CRezMgr* pRezMgr, CRezDir* pParentDir, REZDIRNAME szDirName, DWORD nDirPos, DWORD nDirSize, REZTIME nTime, unsigned int nDirNumHashBins, unsigned int nTypNumHashBins)
  ...
  m_sDirName = new char[strlen(szDirName)+1];
...
//    TRACE("CRezDir::Load Failed! (File is not sorted!)\n");
  ...
  if (m_nItemsSize > 0) {
    ...
    m_pMemBlock = new BYTE[m_nItemsSize];
...
//  ASSERT(m_pRezMgr->m_bReadOnly == FALSE);  // You can't do this if the resource was open read only!
  ...
  pDir = new CRezDir(m_pRezMgr, this, sDir, 0, 0, m_pRezMgr->GetCurTime(), m_pRezMgr->m_nDirNumHashBins, m_pRezMgr->m_nTypNumHashBins);
  ASSERT(pDir != NULL);
  if (pDir == NULL) return NULL;
...
BOOL CRezDir::ReadDirBlock(CBaseRezFile* pRezFile, DWORD Pos, DWORD Size, BOOL bOverwriteItems) {
  ...
  while (pCur < pEnd) {
    ...
    DWORD nEntryType;
    DecodeData(pCur,(BYTE *)&nEntryType,sizeof(DWORD),nPos);
    nPos += sizeof(DWORD);
    // if this is a directory entry
    if (nEntryType == DirectoryEntry) {
      pCur += sizeof(DWORD);
      ...
      DecodeData(pCur,(BYTE *)&Pos,sizeof(DWORD),nPos);
      nPos += sizeof(DWORD);
      pCur += sizeof(DWORD);
      DecodeData(pCur,(BYTE *)&Size,sizeof(DWORD),nPos);
      nPos += sizeof(DWORD);
      pCur += sizeof(DWORD);
      DecodeData(pCur,(BYTE *)&Time,sizeof(DWORD),nPos);
      nPos += sizeof(DWORD);
      pCur += sizeof(DWORD);
      DecodeData(pCur,(BYTE *)&NameLen,sizeof(DWORD),nPos);
      nPos += sizeof(DWORD);
      pCur += sizeof(DWORD);

      // convert dir name
      memset(sDirName,0,sizeof(sDirName));
      DecodeData(pCur,(BYTE *)sDirName,NameLen,nPos);
      nPos += strlen(sDirName)+1;
	  ...
	  if (pDir == NULL) {	
        ...
        pDir = new CRezDir(m_pRezMgr, this, sDirName, Pos, Size, Time, m_pRezMgr->m_nDirNumHashBins, m_pRezMgr->m_nTypNumHashBins);
        ASSERT(pDir != NULL);
    ...
    // if this is a resource item entry
      ...
      ASSERT(nEntryType == ResourceEntry); // Type is not a resource or a directory!!!!
      pCur += sizeof(DWORD);
      ...
      nPos += sizeof(DWORD);
      pCur += sizeof(DWORD);
      DecodeData(pCur,(BYTE *)&ID,sizeof(DWORD),nPos);
      nPos += sizeof(DWORD);
      pCur += sizeof(DWORD);
      DecodeData(pCur,(BYTE *)&Type,sizeof(DWORD),nPos);
      nPos += sizeof(DWORD);
      pCur += sizeof(DWORD);
      DecodeData(pCur,(BYTE *)&NumKeys,sizeof(DWORD),nPos);
      nPos += sizeof(DWORD);
      pCur += sizeof(DWORD);
      DecodeData(pCur,(BYTE *)&NameLen,sizeof(DWORD),nPos);
      nPos += sizeof(DWORD);
      ...
      DecodeData(pCur,(BYTE *)sName,NameLen,nPos);
      nPos += strlen(sName)+1;
      pCur += strlen(sName)+1;
      ...
      memset(sDescription,0,sizeof(sDescription));
      DecodeData(pCur,(BYTE *)sDescription,sizeof(BYTE),nPos);
      nPos += strlen(sDescription)+1;
      pCur += strlen(sDescription)+1;
      if (sDescription[0] == '\0') sDescription[0] = NULL;

      memset(sDescription,0,sizeof(sDescription));
      DecodeData(pCur,(BYTE *)sDescription,32,nPos);
      nPos += 32;
      pCur += 32;
      // convert KeyAry
      if (NumKeys > 0) {
        pKeyAry = new REZKEYVAL[NumKeys];
        ASSERT(pKeyAry != NULL);
...
CRezTyp* CRezDir::GetOrMakeTyp(REZTYPE nType) {
  ...
  if (pTyp == NULL) {
	...
	{
		pTyp = new CRezTyp(nType, this, m_pRezMgr->m_nByIDNumHashBins, m_pRezMgr->m_nByNameNumHashBins);
	}
	else
	{
		pTyp = new CRezTyp(nType, this, m_pRezMgr->m_nByNameNumHashBins);
	}
    ASSERT(pTyp != NULL);
    if (pTyp == NULL) return NULL;
...
BOOL CRezMgr::Open(const char* FileName, BOOL ReadOnly, BOOL CreateNew) {
  ...
  if (IsDirectory(FileName)) {
	...
	pRezFile = new CRezFileDirectoryEmulation(this,m_nMaxOpenFilesInEmulatedDir);
  ...
  // if this is an old file read in header and directories
    ...
    m_pRootDir = new CRezDir(this, NULL, "", m_nRootDirPos, m_nRootDirSize, m_nRootDirTime, m_nDirNumHashBins, m_nTypNumHashBins);
    ASSERT(m_pRootDir != NULL);
...
BOOL CRezDir::WriteDirBlock(CBaseRezFile* pRezFile, DWORD Pos, DWORD* Size) {
  ...
  // write all dir hash table contents out to file
  {
    ...
    while (pDir != NULL) {
      ...
      BYTE Buf[256];
      // write out dir entry
      EncodeData((BYTE *)&Header.Type,Buf,sizeof(Header.Type),nCurPos);
      nCurPos += pRezFile->Write(nCurPos,0,sizeof(Header.Type),Buf);
      EncodeData((BYTE *)&Header.Dir.Pos,Buf,sizeof(Header.Dir.Pos),nCurPos);
      nCurPos += pRezFile->Write(nCurPos,0,sizeof(Header.Dir.Pos),Buf);
      EncodeData((BYTE *)&Header.Dir.Size,Buf,sizeof(Header.Dir.Size),nCurPos);
      nCurPos += pRezFile->Write(nCurPos,0,sizeof(Header.Dir.Size),Buf);
      EncodeData((BYTE *)&Header.Dir.Time,Buf,sizeof(Header.Dir.Time),nCurPos);
      nCurPos += pRezFile->Write(nCurPos,0,sizeof(Header.Dir.Time),Buf);
      NameLen = strlen(pRezDir->m_sDirName);
      EncodeData((BYTE *)&NameLen,Buf,sizeof(NameLen),nCurPos);
      nCurPos += pRezFile->Write(nCurPos,0,sizeof(NameLen),Buf);
      memset(Buf,0,sizeof(Buf));
      if (pRezDir->m_sDirName == NULL) {
        EncodeData(&Zero,Buf,1,nCurPos);
        nCurPos += pRezFile->Write(nCurPos,0,1,&Buf);
      } else {
        NameLen = strlen(pRezDir->m_sDirName);
        EncodeData((BYTE *)pRezDir->m_sDirName,Buf,NameLen,nCurPos);
        nCurPos += pRezFile->Write(nCurPos,0,NameLen+1,Buf);
      }
  ...
  // write all typ hash table contents 
  {
    ...
    while (pTyp != NULL) {
      ...
      while (pItm != NULL) {
        ...
        EncodeData((BYTE *)&Header.Type,Buf,sizeof(Header.Type),nCurPos);
        nCurPos += pRezFile->Write(nCurPos,0,sizeof(Header.Type),Buf);
        EncodeData((BYTE *)&Header.Rez.Pos,Buf,sizeof(Header.Rez.Pos),nCurPos);
        nCurPos += pRezFile->Write(nCurPos,0,sizeof(Header.Rez.Pos),Buf);
        EncodeData((BYTE *)&Header.Rez.Size,Buf,sizeof(Header.Rez.Size),nCurPos);
        nCurPos += pRezFile->Write(nCurPos,0,sizeof(Header.Rez.Size),Buf);
        EncodeData((BYTE *)&Header.Rez.Time,Buf,sizeof(Header.Rez.Time),nCurPos);
        nCurPos += pRezFile->Write(nCurPos,0,sizeof(Header.Rez.Time),Buf);
        EncodeData((BYTE *)&Header.Rez.ID,Buf,sizeof(Header.Rez.ID),nCurPos);
        nCurPos += pRezFile->Write(nCurPos,0,sizeof(Header.Rez.ID),Buf);
        EncodeData((BYTE *)&Header.Rez.Type,Buf,sizeof(Header.Rez.Type),nCurPos);
        nCurPos += pRezFile->Write(nCurPos,0,sizeof(Header.Rez.Type),Buf);
        EncodeData((BYTE *)&Header.Rez.NumKeys,Buf,sizeof(Header.Rez.NumKeys),nCurPos);
        nCurPos += pRezFile->Write(nCurPos,0,sizeof(Header.Rez.NumKeys),Buf);
        NameLen = strlen(pRezItm->m_sName);
        EncodeData((BYTE *)&NameLen,Buf,sizeof(NameLen),nCurPos);
        nCurPos += pRezFile->Write(nCurPos,0,sizeof(NameLen),Buf);
        memset(Buf,0,sizeof(Buf));
        if (pRezItm->m_sName == NULL) {
          EncodeData(&Zero,Buf,1,nCurPos);
          nCurPos += pRezFile->Write(nCurPos,0,1,Buf);
        } else {
          NameLen = strlen(pRezItm->m_sName);
          EncodeData((BYTE *)pRezItm->m_sName,Buf,NameLen,nCurPos);
          nCurPos += pRezFile->Write(nCurPos,0,NameLen+1,Buf);
        }
        memset(Buf,0,sizeof(Buf));
        EncodeData(&Zero,Buf,1,nCurPos);
        nCurPos += pRezFile->Write(nCurPos,0,1,Buf);
        memset(Buf,0,sizeof(Buf));
        EncodeData(Buf,Buf,32,nCurPos);
        nCurPos += pRezFile->Write(nCurPos,0,32,Buf);
...
void CRezMgr::SetDirSeparators(const char* sDirSeparators) {
	...
	m_sDirSeparators = new char[nLen+1];
...
CRezItm* CRezMgr::AllocateRezItm()
{
	...
	if (pNewItem == NULL)
	{
		...
		pNewChunk = new CRezItmChunk;
		if (pNewChunk == NULL) return NULL;

		pNewChunk->m_pRezItmAry = new CRezItm[m_nRezItmChunkSize];

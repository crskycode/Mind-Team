...
// SKY: why we need to export this variable? because vc die a mother to optimize it.
...
DWORD LoadBlobFile(BYTE *pBuffer, BlobFootprint_t *pblobfootprint, void **pv, DWORD dwSize)
{
	...
	{
		// DISCARDED
		VirtualProtect((void *)pSection[j].m_dwVirtualAddress, pSection[j].m_dwVirtualSize, PAGE_EXECUTE_READWRITE, &dwProtect);

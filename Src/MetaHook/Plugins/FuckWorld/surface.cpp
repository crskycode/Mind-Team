...
HFont g_hCurrentFont = vgui::INVALID_FONT;
...
{
	g_iCurrentTextR = col[0];
	g_iCurrentTextG = col[1];
	g_iCurrentTextB = col[2];
	g_iCurrentTextA = col[3];
...
static RECT *g_ScissorRect = (RECT *)0x024978E0;	//rect_024978E0 (3266)
static bool *g_bScissor = (bool *)0x24978F8;	//byte_24978F8
...
void CSurface::DrawUnicodeChar(wchar_t wch)
{
	...
	int abcA, abcB, abcC;
	FontManager().GetCharABCwide(g_hCurrentFont, wch, abcA, abcB ,abcC);

	int wide, tall;
	tall = GetFontTall(g_hCurrentFont);
	...
	if (!FontTextureCache().GetTextureForChar(g_hCurrentFont, 0, wch, &textureID, &texCoords))
		return;

	g_pSurface->DrawSetTexture(textureID);
	g_pfnCSurface_DrawSetTextColor(this, 0, g_iCurrentTextR, g_iCurrentTextG, g_iCurrentTextB, g_iCurrentTextA);
	staticSurface->drawPrintChar(drawX, drawY, wide, tall, texCoords[0], texCoords[1], texCoords[2], texCoords[3]);
	g_pfnCSurface_DrawSetTextColor(this, 0, g_iCurrentTextR, g_iCurrentTextG, g_iCurrentTextB, g_iCurrentTextA);
	g_pSurface->DrawSetTextPos(x + abcB + abcC, y);

	/*RECT rcOut;
	TCoordRect tRect;

	if (!ScissorRect_TCoords(x, y, x+wide, y+tall, texCoords[0], texCoords[1], texCoords[2], texCoords[3], &rcOut, &tRect))
		return;
	...
	qglColor4ub(g_iCurrentTextR, g_iCurrentTextG, g_iCurrentTextB, g_iCurrentTextA);
...
void Surface_InstallHook(void)
{
	...
	g_pSurface = (vgui::ISurface *)engineFactory(VGUI_SURFACE_INTERFACE_VERSION, NULL);
